...
<div class="row">
	...
	<div class="clearfix">
		...
		<?php 
			 $oldtags = array();
			 if($selectedtags) {
				 foreach($selectedtags->result_array() as $tag) {
					 $oldtags[] = $tag['description'];
				 }
			 }
		?>	    
              <select name="tags[]" multiple="multiple" id="normalSelect" size="2">
			<?php foreach($tags->result_array() as $row):?>
				<option value="<?=$row['idtagtypes']?>" <?=(in_array($row['description'],$oldtags) ? " selected='selected'" : '')?>><?=$row['description']?></option>
			<?php endforeach;?>
              </select>
            </div>
          </div><!-- /clearfix -->
