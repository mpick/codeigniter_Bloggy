...
class Info_model extends CI_Model {
    ...
    {
            $where = '';
            if($id) $where = " AND idlink = '{$id}'";
            $query = $this->db->query("SELECT idlink,description,url,note,active
                    FROM link 
    ...
    {
            $where = '';
            if($id) $where = " AND idsoftware = '{$id}'";
            $query = $this->db->query("SELECT idsoftware,description,url,note,active
                    FROM software
                    WHERE active = 1 {$where}
